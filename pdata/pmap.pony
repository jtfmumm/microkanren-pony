...
class val MultiLeafNode[K: (Hashable val & Equatable[K] val), V]
  ...
  fun entries(): List[Entry[K, V]] => _entries
...
class val MapNode[K: (Hashable val & Equatable[K] val), V]
  ...
  fun _insertInArrayAt(arr: Array[Map[K, V]] val, node: Map[K, V], idx: USize): Array[Map[K, V]] val ? =>
    """
    Return a new array with the provided node inserted at provided index, shifting the
    old values at and above the index up one.
    """
  ...
  fun _removeInArrayAt(arr: Array[Map[K, V]] val, idx: USize): Array[Map[K, V]] val ? =>
    """
    Return a new array with the value at the provided index removed and values above the index
    shifted down one.
    """
    var belowArr: USize = 0
    var aboveArr = idx + 1
  ...
  fun pointers(): Array[Map[K, V]] val => _pointers
...
class MapPairs[K: (Hashable val & Equatable[K] val), V]
  var _i: USize = 0
  var _size: USize = 0
  let kvs: Array[(K, val->V)] = kvs.create()
  ...
  fun ref _populate(m: Map[K, V]) =>
    match m
    | let mn: MapNode[K, V] =>
      for node in mn.pointers().values() do
        _populate(node)
      end
    | let l: LeafNode[K, V] =>
      kvs.push((l.key(), l.value()))
    | let multi: MultiLeafNode[K, V] =>
...
primitive _BitOps
  ...
  fun flipIndexedBitOn(bmap: U32, idx: U32): U32 =>
    """
    Return a new bitmap with the index bit set to 1.
    """
    (1 << idx) or bmap

  fun flipIndexedBitOff(bmap: U32, idx: U32): U32 =>
    """
